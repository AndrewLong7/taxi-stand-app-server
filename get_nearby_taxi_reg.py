...
def get_nearby_taxi_stands(
    user_lat, user_lng, user_hour, stand_type, stand_data, lambda_l1=0.1, lambda_l2=0.5
):
    candidates = []
    ...
    for i, stand in enumerate(top_5_by_fscore):
        print(
            f"Rank {i + 1}: Stand ID {stand['stand_id']}, Distance: {stand['distance']:.2f} km, Order Count: {stand['order_count']}, f_score: {stand['f_score']:.2f}"
        )

    print("\nTop 5 Taxi Stands by Distance:")
    for i, stand in enumerate(top_5_by_distance):
        print(
            f"Rank {i + 1}: Stand ID {stand['stand_id']}, Distance: {stand['distance']:.2f} km, Order Count: {stand['order_count']}"
        )
