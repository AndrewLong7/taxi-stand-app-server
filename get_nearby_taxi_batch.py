...
def get_nearby_taxi_stands(user_lat, user_lng, user_hour, stand_type, stand_data, distMethod="google"):
    """结合用户区域仅搜索本区域内的站点"""
    # 确定用户所在区域
    ...
    print(f"用户所在区域：{'Hong Kong Island' if user_region == 'A' else 'New Territories and Kowloon'}")
    ...
    # 批量计算距离
    distances = calculate_distances_batch(user_lat, user_lng, stand_lats, stand_lngs, method=distMethod)

    # 提取订单数量
    order_counts = df_stands['order_count'].apply(lambda orders: orders[str(user_hour).zfill(2)]).values
    ...
    for i, row in top_5_stands.iterrows():
        print(f"站点 ID {row['stand_id']},距离: {row['distance']:.2f} km, "
...
def calculate_distances_batch(user_lat, user_lng, stand_lats, stand_lngs, method="google", batch_size=25, h3_resolution=9):
    if method == "google":
        distances = []
        origins = [(user_lat, user_lng)]
        ...
        for i in range(0, len(destinations), batch_size):
            ...
            try:
                ...
                distances.extend([np.inf] * len(batch_destinations))  # 填充默认值
        return np.array(distances)
    elif method == "h3":
        user_h3 = h3.latlng_to_cell(user_lat, user_lng, h3_resolution)
        stand_h3 = np.array([h3.latlng_to_cell(lat, lng, h3_resolution) for lat, lng in zip(stand_lats, stand_lngs)])
        distances = np.array([h3.grid_distance(user_h3, h3_cell) for h3_cell in stand_h3])
        return distances
    elif method == "osmnx":
        try:
            G = load_road_network(user_lat, user_lng)
            orig_node = ox.distance.nearest_nodes(G, user_lng, user_lat)
            dest_nodes = [ox.distance.nearest_nodes(G, lng, lat) for lat, lng in zip(stand_lats, stand_lngs)]
            distances = [
            ...
            return np.full(len(stand_lats), np.inf)  # 返回默认值
    else:
...
def calculate_f_scores_batch(distances, order_counts, max_distance=10, max_order=70, alpha=5, beta=0.5, gamma=0.3):
    """
    优化后的批量计算 f_score。
    """
    # 矢量化归一化
...
# 获取用户选择
def get_user_distance_choice():
    print("\nPlease select the distance calculation method:")
    print("1: Use OSMnx road network calculation")
    print("2: Use H3 grid calculation")
    print("3: Use Google Maps API calculation")
...
def main():
    ...
    # 加载 JSON 数据
    json_file = get_resource_path("taxi_stands_data_new.json")
    ...
    while True:
        ...
        try:
            ...
            methodChoice = get_user_distance_choice()
        except ValueError:
        ...
        # 获取推荐的站点
        top_5_stand_ids = get_nearby_taxi_stands(user_lat, user_lng, user_hour, stand_type, stand_data, methodChoice)
        ...
        # 提示用户是否继续查询
        continue_query = input("\nDo you want to perform another query? (yes to continue, 'exit' or 'quit' to terminate): ").strip().lower()
