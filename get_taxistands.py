...
def get_nearby_taxi_stands(user_lat, user_lng, user_hour, stand_type, stand_data, hk_geodata):
    """结合用户区域仅搜索本区域内的站点"""
    # 确定用户所在区域
...
def calculate_distances_batch(user_lat, user_lng, stand_lats, stand_lngs, batch_size=25, h3_resolution=9):
    """
    ...
    if h3_distances is None:
        raise ValueError("H3 distances are not loaded. Please ensure they are initialized.")
        ...
        try:
            ...
            for idx, elem in enumerate(results["rows"][0]["elements"]):
                if "distance" in elem and elem["status"] == "OK":
                    ...
                    user_h3 = h3.latlng_to_cell(user_lat, user_lng, h3_resolution)
                    stand_h3 = h3.latlng_to_cell(stand_lat, stand_lng, h3_resolution)
                    batch_distances.append(h3_distances.get((user_h3, stand_h3), float('inf')))
...
def calculate_f_scores_batch(distances, order_counts, max_distance=50, max_order=50, alpha=4, beta=0.5, gamma=0.3):
    """
    优化后的批量计算 f_score。
    """
    # 矢量化归一化
...
def main():
    ...
    while True:
        ...
        # 获取推荐的站点
        top_5_stand_ids = get_nearby_taxi_stands(user_lat, user_lng, user_hour, stand_type, stand_data, hk_geodata)
